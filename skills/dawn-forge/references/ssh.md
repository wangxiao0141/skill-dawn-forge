...
`plan` 自动读取控制机主机名，验证并复用控制机 `~/.ssh/id_ed25519`；两端都不存在时直接创建无 passphrase 的 ED25519 管理 key，并以控制机主机名作为 public key comment，不需要单独确认，也绝不覆盖已有文件。key pair 单边缺失、不匹配或类型错误时停止并诊断。用户要求加密 key 时，改由用户在本地生成并配置 `ssh-agent`。
