...
4. 只通过受控入口传输，不手写 `scp` 或拼接 SSH 命令：
   ...
   `--name` 当前只允许 `clash-subscription-url.txt` 和 `clash-config.yaml`。该 legacy 传输入口尚未适配 Engine 的 `target.json`，不得把 `dawn target bootstrap` 生成的 Target 伪装成旧 identity receipt 后调用；应等到 Engine 对应传输入口完成切换。不得绕过该边界手写 `scp`。
