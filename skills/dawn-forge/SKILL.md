...
- 订阅、密码、token、private key 等秘密不得进入 profile、仓库、文件、命令参数、输出或运行状态。用户明确授权时，只能按 [references/secret-handoff.md](references/secret-handoff.md) 通过控制机剪贴板与 SSH stdin 临时交接。
...
- 需要把订阅 URL、token 或其他 GUI 秘密从控制机交给目标机时，完整读取 [references/secret-handoff.md](references/secret-handoff.md)。
...
2. 展示 SSH、平台 preflight、离线网络引导、秘密剪贴板交接、软件批次、可选代理和目标机 key 阶段中 Agent/用户各自的动作。
...
5. 目标机离线时，把代理客户端的控制机下载、局域网上传、目标机 GUI 配置和联网验证列为批次的第一阶段。

### 6. 批量配置目标机

1. 按目标平台和依赖顺序批量执行已批准的软件，不把 profile 字符串直接拼接为 shell。
2. 代理客户端仅在 profile 明确列出或当前下载被其阻塞且用户同意加入 profile 时处理。目标机离线时，先由控制机从官方渠道下载并经局域网 `scp` 传输；目标机网络门禁通过前不得执行其余联网安装。
3. 用户明确授权交接订阅时，只从控制机剪贴板读取并经 SSH stdin 写入目标机剪贴板；不回显、不落盘、不写入命令参数。用户在目标机 GUI 中完成粘贴和导入。
