...
2. 新目标机按依赖关系分成少量批次，不逐项来回确认：
   - 第一批：引导用户访问目标机、开启 SSH，并一次取得登录账号与局域网 hostname；
   - Agent 检查并选择控制机现有管理公钥；
   - 第二批：给出一段幂等命令，让用户在目标机本地加入公钥并一次回报执行结果；
   - Agent 使用 hostname 完成首次回连。
...
4. 一次询问用户是否给这台电脑使用某个 profile，以及是否需要增删其中的软件；即使只有一个候选也只推荐，不自动选择。
