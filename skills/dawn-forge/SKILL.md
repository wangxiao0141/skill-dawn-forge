...
- 开始时可以没有 SSH 或 alias；结束后使用稳定 SSH alias 或局域网域名管理。
...
- 复用控制机已有 SSH 管理 key；目标机的 GitHub key 和通用 key 必须与管理 key 分开。
...
- 首轮先发现已有 SSH target 和 profile，再让用户选择。不要要求用户预先提供 alias 或 profile 路径。
...
1. 展示已有 SSH target；让用户选择已有电脑或新电脑。新电脑再询问系统、称呼、账号和局域网 hostname。
2. 按 [references/ssh.md](references/ssh.md) 复用管理公钥完成建联，确认 alias，并探测目标机身份与平台。
3. 查找工作区 `profiles/*.json`，逐个运行：

   ```text
   node <skill-directory>/scripts/validate-profile.mjs <profile.json>
   ```

4. 展示匹配 profile 的软件、`settings` 和 `manualTasks`，让用户选择并一次说明增删项。没有有效 profile 时再修复或从空模板创建。
5. 记录确认后的 profile `id`、platform 和 SHA-256。
