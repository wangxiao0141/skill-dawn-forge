...
- 不读取、索要、回显或保存订阅、密码、token、private key 等秘密。
- 优先验证并复用控制机现有 SSH 管理身份；控制机管理身份与目标机外部服务身份不得互相复用。

## 按需读取资源

- 接收、创建或修改 profile 前，完整读取 [references/profile.md](references/profile.md)，并从 [assets/dawn-forge.profile.example.json](assets/dawn-forge.profile.example.json) 复制格式。
...
2. 展示 SSH、平台 preflight、软件批次、可选代理和目标机 key 阶段中 Agent/用户各自的动作。
...
2. 代理客户端仅在 profile 明确列出或当前下载被其阻塞且用户同意加入 profile 时处理。
3. 优先平台原生 package manager，其次使用可验证的官方发布渠道；无法安全自动化时标记 `manual`。
4. 修改现有文件前备份，只修改 Dawn Forge 标记块。
5. 每项安装后重新探测；失败时停止依赖它的后续项，并先诊断再决定恢复。
