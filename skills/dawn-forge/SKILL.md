...
- 只读检查、诊断和非覆盖式 SSH 准备直接执行。目标与配置阶段只在用户输入目标机密码时暂停；key 异常、alias 冲突、host key 或目标身份变化时才停止询问。
...
5. 用户报告完成后，Agent 直接完成 public-key-only 验证、写入 alias 并继续发现 profile，不再请求确认。
