...
1. 不要在启动时要求用户提供 SSH alias 或 profile 路径。先检查用户请求、当前会话、控制机 SSH config 和当前工作区。
2. 展示发现结果：
   - 已有 SSH alias 及其非敏感 HostName/User 摘要；
   - 当前工作区 `profiles/` 下可用 profile 的 `id`、名称、平台，以及软件名称、必装/可选、来源和版本策略摘要。
3. 询问这次要配置哪台电脑以及目标系统。用户已经说明的信息不重复询问；目标不在已有 alias 中时明确说明将引导从零建联。
4. 按目标平台过滤可用 profile。对每个候选说明将安装什么软件、应用哪些 `settings`、保留哪些 `manualTasks`，再询问用户是否给本次目标使用该 profile。即使只有一个匹配项，也只把它标为推荐项并等待确认，不自动采用。
5. 用户本次已经明确指定 profile 时直接使用；没有可用 profile 时，才从空模板创建并询问软件集合。不得把 Skill 的 `assets/` 示例当作候选。
6. 对用户选定的 profile 执行：
...
7. 用户确认后记录 profile SHA-256、`id`、`platform` 和软件数量。SSH 建联完成后再记录 target alias。
8. 发现秘密字段、未知字段、重复软件或平台不兼容来源时停止，不猜测或静默修正。
...
1. 已有 target alias 或局域网域名时，使用真实 OpenSSH 的 `ssh -G <target>` 解析 `HostName`、`User`、`IdentityFile` 和 `IdentitiesOnly`，再尝试 public-key-only 连接。
2. SSH 尚未建立时，不要求用户先创建 alias。引导用户访问目标机，根据已确认平台开启 SSH、确认登录账号和局域网 hostname，并在目标机本地加入控制机管理公钥。
3. 优先检查并复用控制机已有管理身份；没有可用身份时才让用户选择现有 key 或明确确认创建新 key，绝不覆盖。
4. 使用局域网 hostname 完成首次回连后，询问用户希望如何命名这台目标机的稳定 alias；可以提供推荐名称，但必须等待用户确认。
5. 确认 alias、HostName、User 和 IdentityFile 后，在控制机 SSH config 中创建或复用配置；只管理 Dawn Forge 标记块，不覆盖其他配置。
6. 使用最终 alias 完成 public-key-only 验证。首次 host key 使用 `StrictHostKeyChecking=accept-new`，已有记录使用 `yes`；禁止 `no`。
7. 禁用 password、keyboard-interactive、agent 中其他身份和 agent forwarding。
