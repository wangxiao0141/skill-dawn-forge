...
- 安装内容由 profile 唯一决定。优先自动发现工作区已有真实 profile；示例 profile 不构成默认软件集。
...
### 1. 自动发现 target 与 profile

1. 不要在启动时要求用户同时提供 SSH alias 和 profile 路径。先检查用户请求、当前会话、控制机 SSH config 和当前工作区。
2. 按以下顺序解析 profile：
   - 用户本次明确提供的路径；
   - 当前会话已经确认的路径；
   - 当前工作区 `profiles/` 下的真实 JSON profile。
3. `profiles/` 中只有一个候选时直接选择并告知用户；存在多个候选时先按已知目标平台过滤，仍不唯一才让用户选择。不得把 Skill 的 `assets/` 示例当作候选。
4. 没有真实 profile 时，才从空模板创建，并询问用户要安装的软件集合。
5. 执行：
...
6. 记录 profile SHA-256、`id`、`platform` 和软件数量。SSH 建联完成后再记录 target alias。
7. 发现秘密字段、未知字段、重复软件或平台不兼容来源时停止，不猜测或静默修正。
...
2. SSH 尚未建立时，不要求用户先创建 alias。根据 profile 平台引导用户在目标机开启 SSH、确认登录账号和局域网 hostname，并在目标机本地加入控制机管理公钥。
3. 优先检查并复用控制机已有管理身份；没有可用身份时才让用户选择现有 key 或明确确认创建新 key，绝不覆盖。
4. 确认 hostname、User 和 IdentityFile 后，在控制机 SSH config 中创建或复用稳定 alias；只管理 Dawn Forge 标记块，不覆盖其他配置。
5. 使用最终 alias 完成 public-key-only 验证。首次 host key 使用 `StrictHostKeyChecking=accept-new`，已有记录使用 `yes`；禁止 `no`。
6. 禁用 password、keyboard-interactive、agent 中其他身份和 agent forwarding。
