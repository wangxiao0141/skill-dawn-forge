...
2. 引导用户在目标机通过 GUI 开启 SSH，并从系统界面读取登录账号和局域网 hostname；解析失败时再从网络设置读取 IP。不要让用户在目标机 Terminal 粘贴命令。
3. 使用 `scripts/prepare-ssh-bootstrap.mjs` 验证或生成控制机默认 `~/.ssh/id_ed25519`，再根据目标机称呼和 hostname 推荐 alias。
4. 用户确认 alias 后，使用该脚本生成控制机上的 `installKeyCommand`。用户只在控制机终端运行这一条命令并输入目标机密码；脚本直接写入远端 `authorized_keys`。
5. Agent 随后完成 public-key-only 验证并写入 alias。
